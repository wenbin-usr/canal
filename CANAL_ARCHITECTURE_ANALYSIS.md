...
| 多流 / RDS | `GroupEventParser`, `GroupEventSink`, `RdsBinlogEventParserProxy` | `parse/.../group/`, `sink/.../group/` |
...
### 13.10 与 Canal Server batch 语义的差异

| 维度 | Canal Server（TCP/MQ 内嵌） | client-adapter |
|------|----------------------------|----------------|
| 批次标识 | `Message.id` = `batchId`，须 **有序 ack** | `CanalMsgConsumer.getMessage` 内部封装；成功则 **`ack()` 无参** |
| 失败 | `rollback(batchId)` 或 connect 时 `rollback()` | `rollback()` + 重试；`syncSwitch.off` 停同步 |
| 位点提交 | Server `updateCursor` + Store `cleanUntil` | 仅当 `canalMsgConsumer.ack()` 才推进 **Canal** 消费位点 |
| 目标库事务 | 无 | RDB `BatchExecutor.commit`（§60.8）与 Canal ack **两阶段**：先写目标库，再 ack Canal |

**顺序要求**：`writeOut` 内所有 `adapter.sync` 成功后才 `canalMsgConsumer.ack()`；任一分组失败则 rollback Canal，**已写入目标库的行不会自动回滚**（需业务幂等或手工修复）。MQ 模式下 Canal 侧与 §97.7 相同，Adapter 再叠一层目标库一致性。
...
更细的 Store 三指针、`ddlIsolation`（§56.4）、batch 语义见 **§56、§81**；空事务过滤策略见 **§55**。
...
```mermaid
flowchart TB
    subgraph parse_track["解析轨 LogPositionManager"]
        P1[MemoryLogPositionManager 主]
        P2[MetaLogPositionManager 备<br/>min 各 client cursor]
        P1 -->|重启无内存| P2
        P2 --> Parser[findStartPosition / dump]
    end
    subgraph consume_track["消费轨 CanalMetaManager"]
        C1[getWithoutAck → addBatch]
        C2[ack → updateCursor + store.ack]
        C3[PeriodMixed 定时刷 ZK cursor]
        C1 --> C2 --> C3
    end
    Parser --> Store[MemoryEventStoreWithBuffer]
    Store --> C1
```

| 问题 | 查哪条轨 | 典型键/类 |
|------|----------|-----------|
| 重启从哪开始 **dump** | 解析轨 | `FailbackLogPositionManager` §85.6–§85.7、`MetaLogPositionManager` §24 |
| 客户端下次 **get** 从哪读 | 消费轨 | `getCursor` / `getLastestBatch` §68 |
| ZK 上看到的进度 | 消费轨 cursor | `.../cursor` §25.4 |
| Store 能否 **回收** slot | 消费轨 ack 推进 `ackSequence` | §81 |

Server 还提供 `listBatchIds(clientIdentity)`：返回当前未 ack 的 batchId **升序列表**，用于排障「为何 ack 报 not exist」（§72.7）。
...
/otter/canal/destinations/{dest}/1001/
  filter          # UTF-8 订阅表达式（可选）
  cursor          # 已 ack 消费位点（JSON）
  mark/{batchId}  # 未 ack 批次（JSON PositionRange，batchId 10 位补零）
```

> 节点名 **`mark`**（非注释里的 `batch_mark`），见 `ZookeeperPathUtils.BATCH_MARK_NODE`（§99）。
...
### 25.4 cursor / mark 节点 JSON 载荷

`updateCursor` / `addBatch` / `getBatch` 统一用 Fastjson2 序列化，并打开 **`WriteClassName`**，反序列化时可还原 `LogPosition`、`EntryPosition`、`PositionRange` 等具体子类：

```156:169:meta/src/main/java/com/alibaba/otter/canal/meta/ZooKeeperMetaManager.java
    public void updateCursor(ClientIdentity clientIdentity, Position position) throws CanalMetaManagerException {
        String path = ZookeeperPathUtils.getCursorPath(...);
...
| 节点 | 类型 | 典型字段 |
|------|------|----------|
| `cursor` | `LogPosition` | `identity`（`LogIdentity`：源地址）、`postion`（**拼写保留**，`EntryPosition`：`journalName`、`position`、`timestamp`、`serverId`、`gtid`） |
| `mark/{batchId}` | `PositionRange<LogPosition>` | `start` / `ack` / `end`（均为 `LogPosition`）、`endSeq`（Store 槽位，§81.7） |

`ack` 写入 cursor；`end` + `endSeq` 供 Store `cleanUntil`（§81.7）。ZK 上可用 `zkCli.sh get /otter/canal/destinations/{dest}/1001/cursor` 直接查看 JSON。

### 25.5 subscribe / filter 与 batchId 两种来源

**subscribe**：`createPersistent` 客户端目录；有 filter 时写 `filter` 节点（UTF-8 字节），已存在则 `writeData` 覆盖。

**listAllSubscribeInfo**：枚举 `destinations/{dest}` 下 **纯数字** 子目录名作为 `clientId`，再读各 `filter` 拼 `ClientIdentity`（无 filter 节点则 filter=null）。

**batchId 与 mark 子节点**（`ZooKeeperMetaManager` 两套 API，生产默认路径见 §25.6）：

| API | ZK 行为 | batchId 来源 |
|-----|---------|--------------|
| `addBatch(identity, range)` | `createPersistentSequential` 在 `mark/` 下 | 顺序节点名后缀（ZK 序号） |
| `addBatch(identity, range, batchId)` | `createPersistent` → `mark/{10位补零}` | 调用方指定（与内存一致） |

```166:181:meta/src/main/java/com/alibaba/otter/canal/meta/ZooKeeperMetaManager.java
        String batchPath = zkClientx.createPersistentSequential(path + "/", data, true);
        String batchIdString = StringUtils.substringAfterLast(batchPath, "/");
        return ZookeeperPathUtils.getBatchMarkId(batchIdString);
    ...
        zkClientx.createPersistent(path, data, true);  // mark/0000000001
```

```180:182:common/src/main/java/com/alibaba/otter/canal/common/zookeeper/ZookeeperPathUtils.java
    public static String getBatchMarkNode(Long batchId) {
        return StringUtils.leftPad(String.valueOf(batchId.intValue()), 10, '0');
    }
```

**removeBatch** 与内存 `MemoryMetaManager` 相同：必须 ack/rollback **当前最小** batchId，否则 `CanalMetaManagerException`（防乱序丢数据，§9.3）。

### 25.6 与 PeriodMixedMetaManager 的分工（§59）

| 操作 | 运行期（默认） | ZK |
|------|----------------|-----|
| `getWithoutAck` → `addBatch` | 仅内存 `atomicMaxBatchId++` | **不写** mark |
| `ack` → `updateCursor` | 内存立即更新 | **定时** `period` ms 刷 `cursor` |
| `subscribe` / `unsubscribe` | 内存 + 异步任务 | 立即写 ZK |
| 进程启动 `batches` 懒加载 | — | `listAllBatchs` 灌入内存 |

注释明确：**切换主时未 ack 的 batch 可丢弃**，消费者从 cursor 重拉（at-least-once）。`MixedMetaManager` 则每次 `addBatch` 后 `executor.submit` 调 ZK 的 **带 batchId** 写入（§86.3）。
...
> **两种「多流」勿混**：`canal.instance.multi.stream.on` 是 **PolarDB-X binlog-x 单 Parser**（§91.1）；**Manager 多库分组** 才是 `GroupEventParser`（§91.2）。下文 §33.1 仅指后者。

### 33.1 GroupEventParser（Manager 多库分组）

`GroupEventParser` **只代理** 子 Parser 的 `start`/`stop`，不做 binlog 合并；合并发生在 **`GroupEventSink`**（§33.3）。

```15:36:parse/src/main/java/com/alibaba/otter/canal/parse/inbound/group/GroupEventParser.java
...
| 触发方式 | 装配 |
|----------|------|
| **Canal Admin / Manager** | `groupDbAddresses` 多组 → `CanalInstanceWithManager.initEventParser()` 每组建一个 `MysqlEventParser`，再包 `GroupEventParser`（§91.2） |
| **Spring 示例** | `deployer/.../spring/group-instance.xml`：`eventParser` = `GroupEventParser`，`eventParser1/2` 各连不同库 |

每组 Parser：**独立** `parseThread`、独立 `masterInfo`/`standbyInfo`、独立 `LogPositionManager` 实例（`doInitEventParser` 每次 `initLogPositionManager()`），但 **共享** 同一 `destination` 字符串与 **同一** `eventStore` / `eventSink`。
...
### 33.3 GroupEventSink：多库时间线归并

Manager 多分组时 `groupSize > 1` 使用 `GroupEventSink`（非普通 `EntryEventSink`）：

```203:207:instance/manager/.../CanalInstanceWithManager.java
        if (groupSize <= 1) {
...
```47:58:sink/src/main/java/com/alibaba/otter/canal/sink/entry/group/GroupEventSink.java
    protected boolean doSink(List<Event> events) {
        for (Event event : events) {
            barrier.await(event);   // 多库按 executeTime 归并
            if (filterTransactionEntry) {
                super.doSink(Arrays.asList(event));
            } else if (i == size - 1) {
                return super.doSink(events);  // 事务原子：整包再写 Store
            }
        }
    }
```

`TimelineBarrier` / `TimelineTransactionBarrier` 核心规则（类注释）：

- 每个库（通道）向 barrier **提交** 当前 Event 的 `executeTime`；
- 仅当 **groupSize 个通道都提交过** 后，才放行 **全局最小 timestamp** 对应的事件进入 Store；
- 任一库启动慢、主备切换、解析挂掉 → 该通道不再 sink → barrier **永久缺一组** → **其它库也被阻塞**（注释建议用 `CanalEventDownStreamHandler` 监听超时）。

```mermaid
flowchart LR
    P1[Parser1 binlog] --> S1[sink ch1]
    P2[Parser2 binlog] --> S2[sink ch2]
    S1 --> B[TimelineBarrier<br/>按 executeTime 归并]
    S2 --> B
    B --> Store[MemoryEventStoreWithBuffer]
```

**与 §91 的关系**：`GroupEventParser` 管 **拉流并行**；`GroupEventSink` 管 **写入 Store 的全局顺序**。消费侧仍是一个 destination、一套 Meta/Store。

### 33.4 group-instance.xml 与 default-instance 差异

| 项 | `default-instance.xml` | `group-instance.xml`（示例） |
|----|--------------------------|------------------------------|
| `eventParser` | 单个 `MysqlEventParser` | `GroupEventParser` + `eventParser1/2` |
| `metaManager` | `PeriodMixedMetaManager` + ZK | 示例为 **`MemoryMetaManager`**（无 ZK 持久化） |
| `eventSink` | `EntryEventSink` | 仍为 `EntryEventSink`（**未** 演示 `GroupEventSink`；Manager 模式才按 `groupSize` 切换） |

生产 Manager 多库：以 **代码** `GroupEventSink` + `GroupEventParser` 为准；本地 `group-instance.xml` 多用于 **双库联调模板**。
...
### 56.3 覆盖的 DDL EventType

`isDdl` 判定（`MemoryEventStoreWithBuffer`）：

```585:588:store/src/main/java/com/alibaba/otter/canal/store/memory/MemoryEventStoreWithBuffer.java
...
`QUERY` 型 DDL 在 `LogEventConvert` 中已转为上述 `EventType`；**DML 的 ROW** 不受 `ddlIsolation` 影响。

### 56.4 ddlIsolation 分批示例（`doGet`）

配置：`canal.instance.get.ddl.isolation=true`（`default-instance.xml` → `eventStore.ddlIsolation`）。

Ring 上顺序（简化）：

```
seq:  101      102       103        104
      ROW×N    ALTER     ROW×M      CREATE
```

**第一次 get**（`batchSize` 足够大，`ddlIsolation=true`）：

| 步骤 | 行为 |
|------|------|
| 扫描 101… | 收集 ROW，加入 batch |
| 遇到 102 ALTER | `entrys` 非空 → `end = 101`，**不含** ALTER，break |
| 返回 Message | 仅 DML（101 段） |

**第二次 get**（`getLastestBatch` 从上次 `start` 续读或 ack 后从 cursor）：

| 步骤 | 行为 |
|------|------|
| 从 102 起 | `entrys` 空，遇到 DDL → **单独** 加入 batch，`end=102` |
| 返回 Message | **仅 ALTER** |

**第三次 get**：再取 103 段 DML；104 CREATE 再单独一批。

关闭 `ddlIsolation=false` 时：同一 `getWithoutAck` 可能把 ALTER 与前后 DML **混在一个 Message**，Adapter/RDB 需在同一批内保证 DDL 先于 DML 执行，否则易乱序。

**与 ack**：DDL batch 的 `PositionRange.ack` 落在 DDL Event 上（`isDdl` 分支，§56.2），`updateCursor` 可安全跳过混有未执行 DDL 的 DML 位点。

**调参**：下游按批执行 DDL 再 DML 时建议 **开启**；追求单批吞吐、且下游能处理混合包时可关闭（§76.10）。
...
### 59.5 Mixed / Period / File 代码级覆写对照

三类 `*Mixed*` 均 **继承** `MemoryMetaManager`，运行期读写在内存完成；差异仅在 **何时、把什么刷到 ZK 或文件**。`PeriodMixedMetaManager` **未覆写** `addBatch` / `removeBatch` / `clearAllBatchs`，故与纯内存行为一致。

| 方法 | Memory | MixedMetaManager | PeriodMixedMetaManager | FileMixedMetaManager |
|------|--------|------------------|------------------------|----------------------|
| `subscribe` | 内存 | 内存 + **立即** `executor.submit`→ZK | 内存 + 立即 submit→ZK | 内存 + 写本地订阅缓存 |
| `unsubscribe` | 内存 | 内存 + 异步 ZK 递归删 | 同 Mixed | 内存 + 删文件侧记录 |
| `addBatch` | `atomicMaxBatchId++` | 内存 + 异步 `ZK.addBatch(..., batchId)` | **仅内存** | **仅内存** |
| `removeBatch` | 须最小 batchId | 内存删 + 异步 ZK `removeBatch` | **仅内存** | **仅内存** |
| `clearAllBatchs` | 内存清空 | 内存 + 异步 ZK `clearAllBatchs` | **仅内存**（继承） | **仅内存** |
| `updateCursor` | 内存 | 内存 + **每次** 异步 ZK `updateCursor` | 内存 + 加入 `updateCursorTasks` | 内存 + `updateCursorTasks`→`meta.dat` |
| 后台线程 | 无 | `FixedThreadPool(1)` 串行刷 ZK | `scheduleAtFixedRate(period)` 合并刷 cursor | 同 Period，写 `{dataDir}/{dest}/meta.dat` |
| 启动恢复 batch | 无 | ZK `listAllBatchs` 灌入 | 同左（**旧 mark 可能残留**） | **不**恢复 batch（仅 cursor 文件） |

```102:107:meta/src/main/java/com/alibaba/otter/canal/meta/MixedMetaManager.java
    public Long addBatch(...) {
...
**HA 切换后的典型状态**（默认 PeriodMixed）：

| 状态 | ZK `cursor` | ZK `mark/*` | 新节点内存 batch | Store getIndex |
|------|-------------|-------------|------------------|----------------|
| 正常 ack 后 | 已刷（最多延迟 `period`） | 空或旧节点残留 | 空 | 与 cursor 对齐 |
| 未 ack 即宕机 | 可能 **落后** 最后一次 ack | 常为 **空**（运行期未写） | 空 | 可能 **超前** cursor |
| 使用 Mixed 且未 ack | 较新 | **有** mark 节点 | 启动时灌入 | 易与 Store 不一致 → 需 `rollback` |

新 active 节点 `start()` 后：Parser 用 `Failback`+`MetaLogPositionManager` 取 **各 client 最小 cursor**；Consumer 侧未 ack 数据依赖 **Client `rollbackOnConnect` 或 MQ callback rollback**（§97.7），不能指望 ZK mark 恢复（Period 默认）。

### 59.6 与 MixedMetaManager 选型差异（运维）

| 指标 | PeriodMixed（默认） | MixedMetaManager |
|------|---------------------|------------------|
| ZK QPS | 低（cursor 合并写） | 高（每 get 一次 mark 写） |
| failover 未 ack | 丢弃，从 cursor **重复**拉 | mark 可带回，但 **与 Store 常不一致** |
| 异步刷盘滞后 | cursor 最多丢 `period` 窗口 | batch/cursor 均可能 **内存已删、ZK 未删**（单线程 executor 排队） |
| 代码维护 | 生产路径 | 历史实现，新部署一般不切换 |
...
### 60.8 Druid 连接池与并发 BatchExecutor

`RdbAdapter.init` 为 **目标库** 建单例 `DruidDataSource`，默认 **`MaxActive=30`**、`InitialSize/MinIdle=1`、`setAutoCommit(false)` 由 `BatchExecutor` 在取连接后关闭：

```90:97:client-adapter/rdb/src/main/java/com/alibaba/otter/canal/client/adapter/rdb/RdbAdapter.java
        dataSource.setInitialSize(1);
        dataSource.setMinIdle(1);
        dataSource.setMaxActive(30);
```

```127:129:client-adapter/rdb/src/main/java/com/alibaba/otter/canal/client/adapter/rdb/RdbAdapter.java
        rdbSyncService = new RdbSyncService(dataSource,
            threads != null ? Integer.valueOf(threads) : null,
            skipDupException);
```

`RdbSyncService` 默认 **`threads=3`**（yml `threads` 可覆盖）：构造时创建 **3 个** `BatchExecutor` + **3 个** `newSingleThreadExecutor()`，按 `dml.hashCode() % threads` 分区（§60.5）。

```67:84:client-adapter/rdb/src/main/java/com/alibaba/otter/canal/client/adapter/rdb/service/RdbSyncService.java
...
每个 `BatchExecutor` **懒加载** 一条 JDBC `Connection`（`getConnection()` 后 `setAutoCommit(false)`），本批 DML 在该连接上逐条 `PreparedStatement.execute()`，分区线程内 `commit()` / `rollback()`，finally **close** 归还池：

```35:44:client-adapter/rdb/src/main/java/com/alibaba/otter/canal/client/adapter/rdb/support/BatchExecutor.java
...
| 维度 | 行为 |
|------|------|
| 连接数上界 | 并发同步时最多约 **`threads` 条** 同时占用（每分区 1 连接），应满足 `threads ≤ MaxActive`，并为目标库 `max_connections` 留余量 |
| 与 Canal Server | 无关；Adapter 独立进程连 **目标库**，不占用 Canal Store |
| 失败语义 | 任一分区 `rollback()` → 整批 `AdapterProcessor` 不 ack Canal（§13.4），与 MQ 模式 Server rollback 对称 |

`concurrent=true`（mapping 级）时逻辑类似，仍为 **每线程一个 BatchExecutor、一条 Connection**（§60.5）。
...
### 68.6 getLastestBatch 与「重复 get 未 ack」

存在未 ack 批次时，`getWithoutAck` **不**从 cursor 起读，而从 **`getLastestBatch().start`** 续拉（同一逻辑流可拆多次 get）：

```327:350:server/src/main/java/com/alibaba/otter/canal/server/embedded/CanalServerWithEmbedded.java
            PositionRange positionRanges = canalInstance.getMetaManager().getLastestBatch(clientIdentity);
            if (positionRanges != null) {
                events = getEvents(..., positionRanges.getStart(), batchSize, ...);
            } else {
                Position start = metaManager.getCursor(clientIdentity);
                ...
            }
...
| 行为 | 说明 |
|------|------|
| 每次非空 get | 都会 **`addBatch` 新 batchId**，旧 batch 仍留在 `batches` 直到 ack/rollback |
| ack 顺序 | 仍只能先 ack **最小** batchId（§68.3） |
| 错误用法 | 连续 get 不 ack → 堆积多个 batch，Store `getSequence` 已前移，**背压**加剧 |
| 正确用法 | 一批 get → 处理 → ack；或先 `rollback` 再 get |

MQ 模式 `CanalMQStarter` 每轮 get 后等 callback 再下一轮，通常不会叠多个 batch；TCP 客户端若异步多线程 get 需自行串行化。
...
### 72.7 Meta / ZK / Store 专项排障

| 现象 | 根因 | 处理 |
|------|------|------|
| `ack error ... batchId is not exist` | 已 ack/rollback 过、或 batchId 记错 | `listBatchIds` 看当前未 ack 列表；客户端保证 **升序 ack**（§68.3） |
| `batchId:X is not the firstly:Y` | 跳过最小 batch ack/rollback | 先处理 batch Y，或 `rollback()` 清空再消费 |
| ZK `mark` 为空但消费卡住 | **正常**（Period 运行期不写 mark） | 看内存侧日志 `getWithoutAck` / `listBatchIds`；勿以 mark 有无判断是否在途 batch |
| ZK `mark` 很多、重启后异常 | 曾用 Mixed 或旧版本残留 | 清理 `.../mark/*` 或 `rollback`；新节点启动会 `listAllBatchs` 灌入（§59.3） |
| failover 后 **重复** 消费 | ZK cursor 落后 ≤ `period` 或 crash 未刷 | 下游幂等；可调小 `canal.zookeeper.flush.period`（增 ZK 写） |
| failover 后 **丢** 数据（少见） | 误以为 ZK cursor 已含未 ack 数据 | Period 设计 **不** 持久化未 ack；应用 ack 后再切主 |
| cursor JSON 与 MySQL 位点对不上 | `LogPosition.postion` 拼写、或 GTID/文件名混用 | 对照 §25.4 字段；Parser 用 `MetaLogPositionManager.min` 非单 client cursor |
| Store 有数据、get 总空 | 非 active、未 subscribe、或 `getLastestBatch.start` 已越过 | ZK `destinations/{d}/running`；确认 `subscribe`；`rollback` 重置 get 指针（§81.5） |
| MQ 发送成功但反复重投 | callback 未 `commit` 或 `rollback` 后整表回退 | 查 Producer 日志；§97.7 |

**zk 快查**（destination=`example`，MQ clientId=1001）：

```bash
# Server 是否在跑
get /otter/canal/destinations/example/running
# 消费进度（可能落后内存最多 period ms）
get /otter/canal/destinations/example/1001/cursor
# 默认 Period 下常为空
ls /otter/canal/destinations/example/1001/mark
```
...
| 现象 | 根因 | 处理 |
|------|------|------|
| 重启后从 **很旧** binlog 重放 | JVM 内存位点丢，Failback 用 **min(cursor)** | 加快慢消费者 ack；或接受重复；见 §85.7 |
| 重启后 **跳过** 一段 binlog | 误以为 ZK `parse` 是消费位点 | 默认 Failback **不读 parse**；查各 client `cursor` |
| `traffic_delay` 很大、Store 指标正常 | 主从延迟 / 库端大事务 / 心跳少 | 查 MySQL `SHOW SLAVE STATUS`；非加大 `buffer.size` |
| `persistLogPosition` 有、cursor 不动 | Parser 领先 Consumer | 正常；背压看 Store（§76.8） |
| 大事务只 persist 一次 | `buildLastTransactionPosition` 仅取 **TRANSACTIONEND** | 事务内切分多次 flush 但位点落在 END（§84.6） |
| `consume failed` 后重连 | `eventSink.sink` 失败（Store 满等） | `sink_blocking_time`；§76.9 |
| 多库分组后 **全无数据** | `GroupEventSink` 某通道未就绪，barrier 阻塞 | 查各子 Parser 日志；`parser=1,2` 指标；§33.3、§91.6 |
| 多库仅一库有数据 | 同上或 filter 未覆盖另一库 | `subscribe` filter；各 `master.address` |
...
### 76.8 核心参数联动（与 §59 / §81 / §85）

生产调参常需 **成组** 修改，单改一项可能只缓解表象。

| 症状 | 优先看的指标（§92） | 参数组合 | 关联章节 |
|------|---------------------|----------|----------|
| Store 满、Parser 阻塞 | `sink_blocking_time`↑、`produce_seq - consume_seq` 大 | ↑ `memory.buffer.size`；↑ 消费 `batchSize` / 加快 ack | §16、§81 |
| 消费慢、get 有数据 ack 慢 | `ack_delay`↑、`get_delay` 正常 | 下游吞吐；勿连续 get 不 ack（§68.6） | §13.10 |
| MQ 积压、Server 内存涨 | 同上 + MQ send 线程满 | ↑ `canal.mq.canalBatchSize` 谨慎；↑ `send.thread.size`；检查 callback 是否 commit | §97.7 |
| failover 重复消费 | ZK `cursor` 旧 | ↓ `zookeeper.flush.period`（增 ZK 写）；接受 at-least-once 幂等 | §59.2 |
| 进程 crash 丢消费进度 | 仅 cursor 周期刷 | 无法靠 mark 恢复（Period 默认）；业务容忍重复或改 Mixed（少见） | §59.5 |
| Parser 重启「跳位点」 | 内存 LogPosition 空 | Failback 读 **min(client cursor)**（§85.2） | §17.3 |
| 业务延迟大、库无积压 | `traffic_delay`↑ | 主从延迟 / 大事务；非 Store 参数 | §92 Entry |
| GTID 丢事务 | — | `ddlIsolation`、确认 ack 到 TRANSACTIONEND（§81.7） | §56 |

**背压近似（ITEMSIZE）**：

```
未释放槽位 ≈ putSequence - min(getSequence, ackSequence)
当 ≈ buffer.size 时 Parser put 阻塞（§81.2）
```

**MEMSIZE 模式**：背压看 `putMemSize - ackMemSize` 与 `buffer.size × memunit` 量级（§76.1）。

**双 flush.period**（均默认 1000ms，可分别配置）：

| 配置键 | 作用对象 | 路径 |
|--------|----------|------|
| `canal.zookeeper.flush.period` | 消费 cursor → ZK | `.../{clientId}/cursor`（§59） |
| 若 XML 改用 `PeriodMixedLogPositionManager` | 解析位点 → ZK | `.../{dest}/parse`（§85.8） |

默认 **仅** Meta 的 PeriodMixed 刷 cursor；Parser 位点主要在 **内存** `MemoryLogPositionManager`，与消费 cursor **不是同一节点**。

### 76.9 指标 → 调参动作速查

| Prometheus 指标 | 持续异常时的动作 |
|-----------------|------------------|
| `canal_instance_sink_blocking_time` rate>0 | ↑ `memory.buffer.size`；加快消费/ack；减 Parser 侧过滤前开销 |
| `canal_instance_put_delay` 低、`ack_delay` 高 | 客户端/MQ/Adapter 未 ack；查 §72.7、§97.7 |
| `produce_seq - consume_seq` 线性涨 | Store 积压；同第一行 |
| `canal_instance_traffic_delay` 高、Store 正常 | MySQL 主从延迟或大事务；非单纯加大 buffer |
| `canal_instance_publish_blocking_time` | ↑ `parallelBufferSize` / `parallelThreadSize`；CPU 瓶颈 |
| `canal_instance_client_empty_batches` 多 | 正常空轮询；若过高可略增 `getTimeout` 减少空 get |
| 某 `parser=N` 无 binlog 增量、其它正常 | `GroupEventSink` barrier 缺通道 | 修挂掉的子库；§33.3、§91.6 |

### 76.10 ddlIsolation 与下游

| `canal.instance.get.ddl.isolation` | 适用 |
|------------------------------------|------|
| `false`（默认） | 单批吞吐优先；下游能处理 DDL+DML 同包（或仅 ROW） |
| `true` | RDB/自研消费 **每批先 DDL 再 DML**；避免 ALTER 与大量 ROW 同 `Message`（§56.4） |

与 `canal.instance.transaction.size` 独立：后者控制 **Parser 侧** 大事务切分（§54）；`ddlIsolation` 仅影响 **Store.doGet** 如何组 `Message`。
...
> 配置键为 **`canal.instance.get.ddl.isolation`**（非 `store.ddlIsolation`）；分批逻辑与 walkthrough 见 **§56.4**。

`doGet` 遇到 DDL（`ALTER/CREATE/ERASE/RENAME/TRUNCATE/CINDEX/DINDEX`）时：

- 若 batch 内 **尚无 DML**，本批只含该 DDL；
- 若已有 DML，**不包含** 当前 DDL（`end = next - 1`），下次 get 再取。

避免大 DDL 与大量 DML 混在同一 `Message`；GTID 下 DDL batch 的 `ack` 字段同样可作为 cursor 落点（§56.2）。
...
### 81.10 多 batch 叠加与 get 指针

当 Meta 中存在多个未 ack batch（§68.6）时：

- Store 的 `getSequence` 已随每次 `doGet` 前移；
- `ack` 只 `cleanUntil` **当前 remove 的那一个** batch 的 `end/endSeq`；
- 若最小 batch 长期不 ack，`ackSequence` 不前进 → **背压** 持续（`putSequence - ackSequence` 逼近 `bufferSize`）。

`rollback(batchId)` 删除 Meta 中 **最小** batch 后执行 `getSequence = ackSequence`，**所有** 未 ack 的 get 区间在 Store 侧一并作废（非按 batchId 精确回退，§9.3 TODO）。因此 MQ `callback.rollback` 会导致 **同 client 其它在途 batch** 也需重新 get。
...
### 84.8 sink 失败与 Store 背压

`EventTransactionBuffer` flush 后调用 `eventSink.sink`（`EntryEventSink` → `tryPut`）：

```390:394:parse/src/main/java/com/alibaba/otter/canal/parse/inbound/AbstractEventParser.java
...
| 链路 | 行为 |
|------|------|
| Store 满 | `tryPut` false → `applyWait` 退避 → `handler.retry`（§16.3） |
| 持续失败 | `CanalParseException` → `parseThread` finally 清理 → sleep 10~20s 重连（§84.5） |
| 位点 | **仅 sink 成功** 后 `persistLogPosition`；背压期间 Parser 位点 **暂停前进**，避免「位点已推进但 Entry 未进 Store」 |

因此监控上：`sink_blocking_time` 与 `put_delay` 升高时，应优先解决 Store/消费，而非只调 Parser 并行度。
...
### 85.6 Failback 写入路径（默认装配）

```72:81:parse/src/main/java/com/alibaba/otter/canal/parse/index/FailbackLogPositionManager.java
    public void persistLogPosition(String destination, LogPosition logPosition) {
        try {
            primary.persistLogPosition(destination, logPosition);
        } catch (CanalParseException e) {
...
| 组件 | 默认角色 | `persistLogPosition` 实际行为 |
|------|----------|-------------------------------|
| primary `MemoryLogPositionManager` | 每事务 END 写内存 Map | **总是成功**，覆盖 destination 最新解析位点 |
| secondary `MetaLogPositionManager` | 只读兜底 | **`do nothing`**，仅在 primary 抛异常时才会写（默认几乎不会触发） |

因此：**解析位点重启前主要在 JVM 内存**；`getLatestIndexBy` 在内存为空时才读 Meta 各 client **最小 cursor**（§85.3）。这与「每事务写 ZK parse」不同，除非显式替换 `logPositionManager` Bean。
...
sequenceDiagram
    ...
    participant Mem as MemoryLogPositionManager
    participant Meta as MetaLogPositionManager
    participant ZKc as ZK cursor
    participant C as Consumer

    P->>Mem: 每事务 persistLogPosition
    C->>Meta: ack → updateCursor（内存）
    Note over Meta,ZKc: PeriodMixed 最多 delay period 刷 ZKc
    P--xMem: 进程 crash，内存位点丢失
    P->>Meta: getLatestIndexBy Failback
    Meta->>ZKc: min(各 client cursor)
    Note over P: findStartPosition 不低于最慢消费者
```

| 事件 | Parser 起点 | Consumer 起点 |
|------|-------------|---------------|
| 同进程 **优雅重启** | 若内存尚在（未 stop）→ 内存；否则 Failback→min cursor | ZK cursor（可能落后 ack ≤ period） |
| **ZK active 切换**（另一台 Server） | 新 JVM 内存空 → Failback | 新节点 `listAllBatchs` 灌 mark（Period 常为空） |
| 仅 **Consumer** 慢 | 内存位点 **领先** min cursor | 正常，Store 背压 |

**勿混淆**：`MetaLogPositionManager.getLatestIndexBy` 读的是 **消费 cursor**，不是 `MemoryLogPositionManager` 里 Parser 刚 persist 的位点；Parser 领先时 Failback **不会**用内存位点以外的「解析 ZK」除非改配置。

### 85.8 ZK `parse` 节点与 PeriodMixedLogPositionManager

`ZooKeeperLogPositionManager` 使用 **`/otter/canal/destinations/{dest}/parse`**（与 `{clientId}/cursor` 并列，§99）：

```27:45:parse/src/main/java/com/alibaba/otter/canal/parse/index/ZooKeeperLogPositionManager.java
    public LogPosition getLatestIndexBy(String destination) {
...
`PeriodMixedLogPositionManager`（非 default-instance 常见配置）与 Meta 的 PeriodMixed **同构**：`persist` 只写内存 + 加入 `persistTasks`，定时刷 ZK `parse` 节点。

| 节点 | 管理者 | 更新时机 | 默认是否启用 |
|------|--------|----------|--------------|
| `.../parse` | LogPositionManager（ZK/PeriodMixed） | 每事务 flush → 定时刷 ZK | **否**（默认 Failback+Memory） |
| `.../{cid}/cursor` | MetaManager | Client ack → 定时刷 ZK | **是**（PeriodMixedMeta） |

排障时：消费进度看 **cursor**；若启用了 Parser ZK 持久化再看 **parse**（§99.3）。
...
### 86.3 MixedMetaManager vs PeriodMixed（代码级）

> 完整方法覆写矩阵见 **§59.5**；HA 切换状态见 **§59.6**。

| 对比项 | MixedMetaManager | PeriodMixedMetaManager | FileMixedMetaManager |
|--------|------------------|------------------------|----------------------|
| `addBatch`→ZK/文件 | **异步** `addBatch(..., batchId)` | **不写** | **不写** |
| `updateCursor` | 每次 ack **异步** 写 ZK | **定时** 合并写 ZK | **定时** 写 `meta.dat` |
| 启动 batch 恢复 | ZK mark 灌入 | 同左 | **无**（仅 cursor 文件） |
| executor | `newFixedThreadPool(1)` | `scheduleAtFixedRate(period)` | 同 Period |
| 适用 | 低 QPS、需 mark 落盘（少见） | **生产默认** | 单机、无 ZK |

**异步滞后**：Mixed 的 `removeBatch` 先改内存再 `submit` 删 ZK；若进程在 submit 前崩溃，可能出现 **ZK mark 残留** 而内存已空，重启 `listAllBatchs` 灌回 **幽灵 batch** → 对 Store 执行 `rollback` 或清 ZK mark（§72.7）。
...
| 项 | 行为 |
|----|------|
| 路径 | `{canal.instance.global.spring.xml 配置的 dataDir}/{destination}/meta.dat` |
| 持久化内容 | **仅 cursor**（`updateCursorTasks` + `period`，与 PeriodMixed 对 ZK 策略同构） |
| batch | 纯内存；`stop()` 时 `batches.clear()` |
| 适用 | 无 ZK 单机；**不能** 跨机共享消费进度；多进程仍会抢写文件 |

无 ZK 时 **Server HA 选主** 仍可用，但 Meta 进度不随节点迁移，需外置 ZK 或接受重复消费。

### 86.5 ZK 节点与内存结构对应

| 内存（`MemoryMetaManager`） | ZK 路径（§99） | 载荷 |
|---------------------------|----------------|------|
| `destinations` | `.../destinations/{dest}/{clientId}/` 持久目录 | 订阅存在性 |
| `cursors` | `.../{clientId}/cursor` | `Position` JSON + `@type`（§25.4） |
| `batches` | `.../{clientId}/mark/{batchId}` | `PositionRange` JSON；`batchId` 左补零 10 位 |
| filter 字符串 | `.../{clientId}/filter` | UTF-8 字节，非 JSON |

`listAllSubscribeInfo` 只认 **数字** 子目录为 clientId；`1001`（MQ 虚拟客户端）与业务 client 并列，各自独立 cursor/mark。

### 86.6 选型与 ZK 写路径（决策简表）

```mermaid
flowchart TD
    Q{需要 ZK 集群 Meta?}
    Q -->|否| F[FileMixedMetaManager<br/>meta.dat 定时刷盘]
    Q -->|是| P{生产 QPS}
    P -->|高| PM[PeriodMixedMetaManager<br/>batch 仅内存 + cursor 周期刷 ZK]
    P -->|低、要快落盘 batch| MM[MixedMetaManager<br/>每次 batch 异步写 mark 节点]
    PM --> ZK[ZooKeeperMetaManager]
    MM --> ZK
```

| 场景 | 推荐 | 注意 |
|------|------|------|
| 默认 `default-instance.xml` | `PeriodMixedMetaManager` | failover 窗口内 cursor 最多丢 `period` ms；**未 ack batch 不恢复** |
| 调试 / 单测 | `MemoryMetaManager` | 重启全丢 |
| 强依赖 ZK 恢复未 ack 批次 | `MixedMetaManager`（非默认） | ZK 写放大；`addBatch` 用 **指定 batchId** 与内存对齐 |
| 直读 ZK 排障 | `ZooKeeperMetaManager` | 一般不单独作为对外 Bean |

与 **§99** 路径 API 对照：`getCursorPath` / `getBatchMarkWithIdPath` / `getFilterPath` 与上表一一对应。
...
### 91.6 GroupEventSink 阻塞与解析位点

**归并阻塞**（§33.3）：某子 Parser 宕机或长期无事件 → `TimelineBarrier` 等不满 `groupSize` → **整 destination 无新数据进 Store**。排障时应对 **每个** `parser=1,2,...` 看 `received_binlog_bytes` 是否还在涨（§92）。

**LogPositionManager 实例**：`doInitEventParser` 对每个子 Parser 调用 `initLogPositionManager()`，各子 Parser 持有 **独立** `MemoryLogPositionManager`（各自 Map）。运行期各线程 `persistLogPosition(destination, …)` 写入 **各自内存**，互不覆盖。

**重启 Failback**：内存清空后 `getLatestIndexBy` 走 **同一** `MetaLogPositionManager`（整 instance 一个 `metaManager`），取 **所有 client 最小 cursor**（§85.3）——**不区分** 哪个子库 Parser；多库物理位点不同步时，需依赖各库 `masterPosition` 配置或人工校准。

**filter 下发**：`subscribeChange` 遍历子 Parser 设置相同 `AviaterRegexFilter`（§94）；各库仍需 filter 能覆盖本库表名。

```mermaid
flowchart TB
    subgraph parsers
        P1[Parser1 MemoryLogPosition]
        P2[Parser2 MemoryLogPosition]
    end
    subgraph shared
        GS[GroupEventSink + Barrier]
        ST[Store]
        Meta[MetaManager cursor]
    end
    P1 --> GS
    P2 --> GS
    GS --> ST
    Meta -.->|Failback min cursor| P1
    Meta -.->|Failback min cursor| P2
```
...
结合 **§76.8–§76.9** 调大 `buffer.size` 或加快消费 / 增大 `batchSize`。
...
### 97.7 Producer 失败时的 rollback 闭环

MQ 投递与 TCP 消费共用 `CanalServerWithEmbedded` 的 Meta + Store 语义；差异在于 ack 由 **Producer Callback** 触发，而非客户端 `CLIENTACK`。

```188:198:server/src/main/java/com/alibaba/otter/canal/server/CanalMQStarter.java
                            canalMQProducer.send(canalDestination, message, new Callback() {
                                public void commit() {
                                    canalServer.ack(clientIdentity, batchId);
                                }
...
```466:489:server/src/main/java/com/alibaba/otter/canal/server/embedded/CanalServerWithEmbedded.java
...
sequenceDiagram
    ...
    participant E as CanalServerWithEmbedded
    participant M as MetaManager
    participant S as MemoryEventStoreWithBuffer
    participant P as CanalMQProducer
    W->>E: getWithoutAck(1001) → batchId
    E->>M: addBatch(PositionRange)
    E->>S: doGet（getIndex 前移）
    W->>P: send(message, Callback)
    alt 发送成功 commit
        P->>W: callback.commit()
        W->>E: ack(batchId)
        E->>M: removeBatch + updateCursor
        E->>S: ack(end, endSeq) cleanUntil
    else 发送失败 rollback
        P->>W: callback.rollback()
        W->>E: rollback(batchId)
        E->>M: removeBatch（须为最小 batchId）
        E->>S: rollback() 整表回退 getIndex
    end
```

| 步骤 | 作用 |
|------|------|
| `removeBatch` | 去掉该 batch 的 Meta 记录；乱序 rollback 同 ack 一样抛错（§9.3） |
| `eventStore.rollback()` | **不**按 batch 精确回退（源码 TODO）；`getIndex` 回到上次 **ack** 位置，本 batch 及之后未 ack 数据可再次 `getWithoutAck` |
| 与背压 | rollback 后 Store 可再次堆积；MQ 发送慢会导致 1001 占满 ring（§5.6、§19） |

Kafka/RocketMQ Producer 侧：`flush()` + 全部分区 `Future` 成功才 `commit()`，否则 `rollback()`（§31.4、§106）。**同一条消息** 若 MQ 已发出但 callback 前进程崩溃，仍可能重复投递，下游需幂等。
...
| `getParsePath(dest)` | `.../{dest}/parse` | **Parser** 解析位点（§85.8） |
| `getFilterPath(dest, clientId)` | `.../{clientId}/filter` | 过滤表达式 |
| `getCanalClusterNode(node)` | `/otter/canal/cluster/{ip:port}` | 全局 Server 登记 |

`cursor` / `mark/*` 节点字节为 Fastjson2 + `WriteClassName` 的 JSON，字段含义见 **§25.4**；`PeriodMixed` 运行期通常 **不写** `mark`（§25.6）。

### 99.3 双轨位点：parse vs cursor（ZK 上）

| 对比 | `destinations/{d}/parse` | `destinations/{d}/{clientId}/cursor` |
|------|--------------------------|--------------------------------------|
| 语义 | Parser 已解析并 flush 的事务位点 | 某 client **已 ack** 的安全消费位点 |
| 默认谁写 | 通常 **不写**（Memory primary） | PeriodMixed 定时刷（§59） |
| Failback 读谁 | **不读 parse**；读 min(**cursor**) | 各 client 独立 |
| 排障 | 仅当启用 ZK/PeriodMixed **LogPositionManager** | 消费进度、HA 恢复主看此节点 |

同一 destination 上 **parse 可新于 cursor**（Parser 快于 Consumer），属正常；**cursor 新于 parse** 在默认配置下少见（除非手动改 ZK）。
...
# Parser 位点（仅启用 ZK LogPositionManager 时有数据）
...
| Mixed vs Period / HA | §59.5–§59.6 | §86.3、§25.6 |
| Meta/ZK 排障 | §72.7 | §99、§68.6 |
| 双位点 Parser/Meta | §17.3 | §24、§85 |
| get 叠 batch / Store rollback | §68.6、§81.10 | §97.7 |
| Adapter 与 Canal ack | §13.10 | §60.8、§13.4 |
...
| Adapter | §13（§13.10 ack） | §82、§60、§102 |
| RDS | §95 | §33 |
| 多库 Group / barrier | §33.3、§91.6 | §72、§76.9 |
| ddlIsolation 分批 | §56.4 | §76.10、§81.8 |
| 配置项 / 调参联动 | §98、§76.8–§76.10 | §92 |
| 解析位点 Failback | §85.6–§85.8 | §17.3、§99.3 |
| 排障 | §72（§72.7 Meta、§72.8 解析位点） | §84.8、§59.6 |
| 监控 | §23 | §92 |
| Manager 配置 | §35.5 | §107 |
| Entry 协议 | §46 | §45 |
...
| Meta 实现 | §17、§59（§59.5–59.6 代码对比） | §25、§86、§68、§72.7 | |
| ZK Meta 节点载荷 | **§25.4–§25.6** | §86.5–§86.6、§99 | cursor/mark/filter |
| Meta/ZK 排障 | **§72.7** | §59.6、§68.6 | mark 空、failover 重复 |
| Adapter 双阶段 ack | **§13.10** | §60.8、§13.4 | 目标库 vs Canal |
| MQ rollback 闭环 | **§97.7** | §9.3、§81.5、§19 | Producer Callback |
| Parser 位点 | §24、§85（§85.6–§85.8） | §104、§72.8、§99.3 |
| 调参联动 / 指标 | **§76.8–§76.10** | §92、§5.6 |
| GroupEventSink 归并 | **§33.3** | §91.6、§72 |
| ddlIsolation | **§56.4** | §81.8、§76.10 |
| sink 背压与位点 | **§84.8** | §16.3、§76.8 |
| Prometheus | §23 | §92 | |
| MQ 虚拟客户端 | §19 | §57、§97 | |
| Session / Netty | §15 | §80、§87 | |
| 多流 / Group 归并 | §33、§91（§91.6） | §95、group-instance |
| ddlIsolation 分批 | **§56.4** | §81.8、§76.10 | |
...
| Meta ZK 刷盘 | **§59** | §25、§86 |
| RDB SQL 落地 | **§60**（§60.8 连接池） | §13.7、§64 | parse4Dml / BatchExecutor |
| 全链路速查 | §103 | §36、§72、§76 | |
